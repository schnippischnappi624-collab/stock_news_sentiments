...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "fetch-feeds":
        return fetch_feeds_command(base_url=args.base_url, force=bool(args.force))
    if args.cmd == "parse-feeds":
        return parse_feeds_command(run_id=args.run_id)
    if args.cmd == "build-shortlist":
        return build_shortlist_command(run_id=args.run_id, extra_candidates=int(args.extra_candidates))
    if args.cmd == "update-news-cache":
        return update_news_cache_command(run_id=args.run_id)
